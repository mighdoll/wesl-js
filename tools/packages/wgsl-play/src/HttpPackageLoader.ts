...
async function fetchSourcePackage(
  ...
  while (pending.length > 0) {
    ...
    // Find internal references using findUnboundIdents
...
function findInternalReferences(source: string, pkgName: string): string[] {
  ...
  for (const path of unbound) {
    if (path[0] === pkgName && path.length > 1) {
      // Internal reference like ['lygia', 'math', 'mod289']
      internalRefs.push(path.slice(1).join("/"));
    }
  }
  return internalRefs;
}
...
async function fetchPackageJson(
  ...
  const cacheKey = `${packageBase}/${pkgName}`;
