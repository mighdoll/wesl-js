...
const pipelineSrc = `@test @entry do test_pipeline() {}`;
...
test("findDoBlocks returns all `do` blocks", () => {
  ...
  expect(blocks[0].isEntry).toBe(true);
});

test("findDoBlockTests filters @test @entry blocks", () => {
...
test("a helper do block (no @test) is not a test", () => {
  const ast = parseTest(`
    ...
    @test @entry do real_test() { step(1, 1, 1); }
  `);
